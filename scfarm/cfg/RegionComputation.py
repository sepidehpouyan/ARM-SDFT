class RegionComputation:
    ...
    def __init__(self, program, region_then, region_else, nemesis_region_then, nemesis_region_else, junction):
    ...
    def start_computation(self, instruction):
        ...
        self.junction, self.region_then, self.nemesis_region_then = self.__compute(self.missing_then, self.nemesis_missing_then, \
                                                            self.region_then, self.nemesis_region_then, self.junction, instruction)

        #print("self.region then: ", self.region_then)

        self.junction, self.region_else, self.nemesis_region_else = self.__compute(self.missing_else, self.nemesis_missing_else, \
                                                            self.region_else, self.nemesis_region_else, self.junction, instruction)

        #print("self.region else: ", self.region_else)

        return self.region_then, self.nemesis_region_then, self.region_else, self.nemesis_region_else, self.junction

    def __compute(self, missing, nemesis_missing, region, nemesis_region, junction, instruction):
        #print("----------------------------- :", region)
        #junction = set()
        while len(missing) > 0:
