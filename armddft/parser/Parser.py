...
class Parser:
    ...
    def parse_elf(file, elf, starting_function, starting_function_address, size, section_name,
        ...
        while (insoff < len(code)):
            ...
            mnemonic, length, address, op_num, src_op, dst_op, clock, condition, setflag, disp, \
                index_reg, base_reg = abs_ins.disasm(code[insoff : insoff + 4], base_address + insoff)

            instr = InstructionFactory.get_instruction(mnemonic, function=current_func)
            instr.get_info(length, address, op_num, src_op, dst_op,clock, condition, setflag, disp, 
